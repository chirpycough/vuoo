        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-lg">
          ...
          <div className="glass-card rounded-2xl overflow-hidden border border-white/10 shadow-2xl shadow-black/50">
            <div className="flex border-b border-white/5">
              <button onClick={() => setActiveTab("auto")} className={`flex-1 py-4 text-sm font-medium transition-colors relative ${activeTab === "auto" ? "text-cyan-400" : "text-slate-400 hover:text-slate-200"}`}>
                Automatic
                {activeTab === "auto" && <motion.div layoutId="tab-indicator" className="absolute bottom-0 left-0 right-0 h-0.5 bg-cyan-400" />}
              </button>
              <button onClick={() => setActiveTab("manual")} className={`flex-1 py-4 text-sm font-medium transition-colors relative ${activeTab === "manual" ? "text-cyan-400" : "text-slate-400 hover:text-slate-200"}`}>
                Manual
                {activeTab === "manual" && <motion.div layoutId="tab-indicator" className="absolute bottom-0 left-0 right-0 h-0.5 bg-cyan-400" />}
              </button>
            </div>
                ...
                {activeTab === "auto" ? (
                  <motion.div key="auto" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="h-full flex flex-col items-center justify-center text-center space-y-6 py-8">
                    ...
                    <div className="space-y-2">
                      <h3 className="text-lg font-semibold text-white">{isAutoConnecting ? "Initializing Secure Channel..." : "Automatic Connection"}</h3>
                      <p className="text-slate-400 text-sm max-w-xs mx-auto">{isAutoConnecting ? "Verifying wallet status and establishing end-to-end encryption." : "Automatically detect and connect to your browser extension or mobile wallet."}</p>
                    </div>
                    <Button onClick={handleAutoConnect} disabled={isAutoConnecting} className="w-full max-w-xs bg-cyan-500 hover:bg-cyan-600 text-white font-semibold h-12 rounded-xl mt-4 disabled:opacity-70">
                    ...
                    {isSuccess ? (
                      ...
                      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                        <div className="p-4 rounded-xl bg-blue-500/10 border border-blue-500/20 flex gap-3 mb-6">
                          ...
                          <p className="text-sm text-blue-200/80">Enter your wallet details manually to establish a secure, encrypted connection.</p>
                        </div>
                        <div className="space-y-2">
                        ...
                        <Button type="submit" disabled={isLoading} className="w-full h-12 bg-gradient-to-r from-cyan-500 to-purple-600 hover:from-cyan-400 hover:to-purple-500 text-white font-bold rounded-xl shadow-lg shadow-purple-500/20 transition-all duration-300 mt-4">
                          {isLoading ? <span className="flex items-center gap-2"><RefreshCw className="w-4 h-4 animate-spin" /> Verifying...</span> : "Connect Wallet"}
                        </Button>
                      </form>
                    )}
                  </motion.div>
